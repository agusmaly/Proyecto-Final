...
const mostrarResultados = (puntaje, preguntasIncorrectas, nombre, edad, intentos) => {
    ...
    obtenerPersonaje(puntaje).then(personaje => {
        ...
        if (personaje) {
            const personajeInfo = `
                ¡Felicidades! Según tu puntaje, tu personaje es ${personaje.name}.
...
function iniciarQuiz(nombre, edad) {
    ...
    const quiz = () => {
        ...
        const preguntas = [
            { pregunta: "¿Cuántas letras tiene la palabra 'Australopithecus'?", respuestaCorrecta: 16 },
            { pregunta: "¿Cuántos colores tiene la bandera de Sudáfrica?", respuestaCorrecta: 4 }, //La respuesta era 6. No contemplé el hecho de que ByN son valores
            { pregunta: "¿Cuántos lados tiene un hexágono?", respuestaCorrecta: 6 },
            { pregunta: "¿Cuánto es 7 * 7?", respuestaCorrecta: 49 },
            { pregunta: "¿Cuántas Copas del Mundo tiene Argentina?", respuestaCorrecta: 3 },
        ];
...
// Entiendo la correción de evitar declarar funciones dentro de otras, intenté separar las funciones manejarPreguntas, verificarRespuesta y mostrarResultados pero no puedo separarlo sin romper la acumulación de intentos de nuevoJugador
